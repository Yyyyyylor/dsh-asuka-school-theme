...
The release contains three 16:9 WebP wallpapers, each under the 1.5 MB budget.
All use a right-side subject and left-side negative space so the conversation
...
Private development artwork belongs only in `assets/private/`; the package
check rejects any private file other than `.gitkeep`.
