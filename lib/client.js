...
var WALLPAPER_ASSET_VERSION = "2.0.0";
...
/* DSH's CodeBlock exposes .md-code-block as its stable outer wrapper. Keep
   every banner/content surface inside it so opaque token backgrounds cannot
   square off or leak beyond the shared edge. */
body[data-asuka-school-theme] .md-code-block {
  --asuka-code-block-radius: 10px;
  position: relative;
  isolation: isolate;
  overflow: hidden;
  border: 1px solid var(--dsw-alias-border-l2);
...
/* The first child is CodeBlock's sticky banner wrapper; its child is the
   language/copy toolbar. Both must paint their own opaque, clipped surface. */
body[data-asuka-school-theme] .md-code-block > :first-child {
  position: sticky;
  top: 0;
  z-index: 2;
  overflow: hidden;
...
body[data-asuka-school-theme] .md-code-block > :first-child > :first-child {
  overflow: hidden;
  border-radius: inherit;
  background: transparent;
  background-clip: padding-box;
}
/* Highlighted blocks place <pre> inside a body wrapper; plain blocks use the
   same <pre> directly. The wrapper clips backgrounds, while <pre> remains the
   scroll owner for horizontal and vertical code navigation. */
body[data-asuka-school-theme] .md-code-block > :last-child:not(pre) {
  position: relative;
  z-index: 0;
  overflow: hidden;
  border-radius: var(--asuka-code-block-radius);
...
body[data-asuka-school-theme] .md-code-block :where(pre) {
